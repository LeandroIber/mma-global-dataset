...
OUTPUT_FILE = "clean_sherdog_jungle_fight.csv"
...
def _merge_fighter(df, fighters, side, key):
    bio_cols = [c for c in fighters.columns if c != "fighter_url"]
...
def main():
    ...
    if len(df) != n_fights:
        print(f"[WARN] contagem mudou após o merge: {n_fights} -> {len(df)}")

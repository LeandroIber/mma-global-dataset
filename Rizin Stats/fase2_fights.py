...
# Fase agnostica de organizacao: le raw_events.csv (gerado pela fase 1) e
# visita cada event_url. Os seletores abaixo foram verificados contra o HTML
# de um evento do RIZIN (lutas_pag_principal.html) e batem 100%:
#   - div.fight_card / div.fighter.left_side / div.fighter.right_side
#   - div.versus > span.weight_class
#   - table.fight_card_resume (Match / Method / Referee / Round / Time)
#   - table.new_table.result com <tr itemtype="http://schema.org/Event">
    ...
    if fc:
        ...
        if resume:
            for td in resume.find_all("td"):
                ...
                if "Match" in txt:
                    try:
                        order = int(txt.replace("Match", "").strip())
                    except ValueError:
                        pass
                elif "Method" in txt:
                    method = txt.replace("Method", "").strip()
                elif "Referee" in txt:
                    referee = txt.replace("Referee", "").strip()
                    a_tag = td.find("a")
                    if a_tag:
                        ref_url = get_abs_url(a_tag.get("href", ""))
                elif "Round" in txt:
                    round_num = txt.replace("Round", "").strip()
                elif "Time" in txt:
                    match_time = txt.replace("Time", "").strip()
...
def main():
    try:
        events_df = pd.read_csv(data_path("raw_events.csv"))
        urls = events_df["event_url"].dropna().unique()
    except Exception as exc:
        sys.exit(f"Erro ao ler raw_events.csv: {exc}")
    ...
    # Status preliminar. A fase4 reconcilia com a data do evento para separar
    # REALIZADO de AGENDADO (o Sherdog publica fight card de eventos futuros).
    for i, url in enumerate(urls, 1):
        ...
        if html is None:
            ...
            fights = parse_fights(html, event_url=url)
            if fights:
                for f in fights:
                    f["event_status"] = "REALIZADO"
                rows.extend(fights)
            else:
                row = base_empty.copy()
                row.update({"event_url": url, "event_status": "SEM_LUTAS"})
                rows.append(row)

        time.sleep(CRAWL_DELAY)

    if not rows:
        sys.exit("Nenhuma luta extraida.")

    out = data_path("raw_fights.csv")
    pd.DataFrame(rows).to_csv(out, index=False, encoding="utf-8")

    erros = sum(1 for r in rows if r.get("event_status") == "ERRO_FETCH")
    print(f"Sucesso! {len(rows)} linhas salvas em {out}.")
    if erros:
        print(f"[WARN] {erros} evento(s) falharam no download (status ERRO_FETCH).")
